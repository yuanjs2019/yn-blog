...
<div style="width:100%;padding-top:70px;">
    <div class="ss-title"><i class="fa fa-tag"></i>系列 >${seriesTitleName}</div>
</div>
    ...
    $(function () {
        ...
        if(pages < 1){
            ...
            for(let i = 1, len = pages+1; i < len; i++) {
                if(i == pageNum){
                    html+='<a href="${re.contextPath}/series/'+i+'/${seriesTitleCode}"> <li class="item -active">'+i+'</li></a>'
                }else{
                    html+='<a href="${re.contextPath}/series/'+i+'/${seriesTitleCode}"><li class="item ">'+i+'</li></a>';
                }
            }
         }else{
            if( pageNum ==1) {
                 html += '<a href="${re.contextPath}/series/1/${seriesTitleCode}"><li class="item -active">1</li></a>'
                       + '<a href="${re.contextPath}/series/2/${seriesTitleCode}"><li class="item ">2</li></a>'
                       + '<a href="${re.contextPath}/series/3/${seriesTitleCode}"><li class="item ">3</li></a>'
              }else if(pageNum == pages){
                   html +='<a href="${re.contextPath}/series/'+(pages-2)+'/${seriesTitleCode}"><li class="item ">'+(pages-2)+'</li></a>'
                         +'<a href="${re.contextPath}/series/'+(pages-1)+'/${seriesTitleCode}"><li class="item ">'+(pages-1)+'</li></a>'
                         +'<a href="${re.contextPath}/series/'+pages+'/${seriesTitleCode}"><li class="item -active">'+pages+'</li></a>';
              }else{
                 html +='<a href="${re.contextPath}/series/'+(pageNum-1)+'/${seriesTitleCode}"><li class="item ">'+(pageNum-1)+'</li></a>'
                      +'<a href="${re.contextPath}/series/'+pageNum+'/${seriesTitleCode}"><li class="item -active">'+pageNum+'</li></a>'
                      +'<a href="${re.contextPath}/series/'+(pageNum+1)+'/${seriesTitleCode}"><li class="item ">'+(pageNum+1)+'</li></a>';
             }
         }
        html += '</ul>';
